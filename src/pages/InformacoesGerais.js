...
// 1. Importamos as ferramentas do Storage
...
function InformacoesGerais() {
  ...
  const [themeColor, setThemeColor] = useState('theme-rose'); // Cor padrão
  const [imageUrl, setImageUrl] = useState(''); // Guarda o link da imagem
  const [uploadProgress, setUploadProgress] = useState(0); // Para a barra de progresso
  const [imageFile, setImageFile] = useState(null); // Guarda o arquivo de imagem selecionado

  const [mensagem, setMensagem] = useState('');
  const [copySuccess, setCopySuccess] = useState('');

  // useEffect agora busca os novos campos de personalização
  useEffect(() => {
    if (user) {
  ...
  // Função para salvar as informações de texto e o tema de cor
  ...
  const handleImageUpload = () => {
    ...
    // Cria uma referência para o arquivo no Storage. Ex: 'fotos/UID_DO_USUARIO/headerImage.jpg'
    ...
    uploadTask.on('state_changed', 
      ...
      () => {
        // Quando o upload termina com sucesso
        getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
          setImageUrl(downloadURL); // Atualiza a imagem na tela
          const userDocRef = doc(db, 'users', user.uid);
          await updateDoc(userDocRef, { imageUrl: downloadURL }); // Salva o link no Firestore
          setUploadProgress(0);
          setImageFile(null);
  ...
  return (
    <div className="space-y-8">
        ...
        <form onSubmit={handleSaveInfo} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-4">
              <button type="button" onClick={() => setThemeColor('theme-rose')} className={`w-10 h-10 rounded-full bg-rose-500 ${themeColor === 'theme-rose' && 'ring-2 ring-offset-2 ring-rose-500'}`}></button>
              <button type="button" onClick={() => setThemeColor('theme-sky')} className={`w-10 h-10 rounded-full bg-sky-500 ${themeColor === 'theme-sky' && 'ring-2 ring-offset-2 ring-sky-500'}`}></button>
              <button type="button" onClick={() => setThemeColor('theme-emerald')} className={`w-10 h-10 rounded-full bg-emerald-500 ${themeColor === 'theme-emerald' && 'ring-2 ring-offset-2 ring-emerald-500'}`}></button>
              <button type="button" onClick={() => setThemeColor('theme-amber')} className={`w-10 h-10 rounded-full bg-amber-500 ${themeColor === 'theme-amber' && 'ring-2 ring-offset-2 ring-amber-500'}`}></button>
            </div>
          </div>
      ...
      <div className="bg-white p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-playfair font-bold mb-4 text-pink-800">Foto Principal do Site</h2>
        {imageUrl && <img src={imageUrl} alt="Foto do casal" className="w-full h-48 object-cover rounded-lg mb-4" />}
        <label htmlFor="upload-foto" className="block text-gray-700 font-semibold mb-2">Escolher nova foto:</label>
        <input id="upload-foto" type="file" onChange={(e) => setImageFile(e.target.files[0])} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-pink-50 file:text-pink-700 hover:file:bg-pink-100"/>
        {uploadProgress > 0 && <div className="w-full bg-gray-200 rounded-full h-2.5 mt-4"><div className="bg-pink-600 h-2.5 rounded-full" style={{ width: `${uploadProgress}%` }}></div></div>}
        <button onClick={handleImageUpload} disabled={!imageFile || uploadProgress > 0} className="mt-4 w-full bg-green-500 text-white font-bold py-3 rounded-lg hover:bg-green-600 disabled:bg-gray-300">
          {uploadProgress > 0 ? `Enviando... ${Math.round(uploadProgress)}%` : 'Enviar Imagem'}
        </button>
      </div>
